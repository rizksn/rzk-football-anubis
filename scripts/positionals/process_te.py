...
OUT_PATH = Path("anubis/data/processed/nfl/nfl_player_te_{year}.processed.json")

# Source-specific key mappings
REC_MAP = {
    "rec": "rec",
    "yds": "rec_yds",
    "td": "rec_td",
...
# All keys expected in final player dicts
ALL_STAT_KEYS = set(REC_MAP.values()) | set(PASS_MAP.values()) | set(RUSH_MAP.values())

def process_te_stats(year: int):
    ...
    for source_data, mapping in [
        ...
        for player in source_data:
            ...
            if not match or match["position"] != "TE":
                continue
    ...
    print(f"✅ Processed {len(final)} TEs → {output_path.name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate combined TE stats for a given season")
    parser.add_argument("--year", type=int, required=True, help="Season year (e.g. 2024)")
    args = parser.parse_args()
    process_te_stats(args.year)