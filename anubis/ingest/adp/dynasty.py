...
async def load_dynasty_adp():
    base_path = os.path.join(os.path.dirname(__file__), "../../data/raw/adp/draftsharks/dynasty")
    ...
    async with async_session() as session:
        for file in files:
            ...
            with open(os.path.join(base_path, file)) as f:
                ...
                await session.execute(insert(draftsharks_dynasty_2025), records)
                print(f"✅ Inserted {len(records)} from {file}")
        await session.commit()

if __name__ == "__main__":
    import asyncio
    asyncio.run(load_dynasty_adp())